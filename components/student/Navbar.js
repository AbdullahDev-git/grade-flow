...
import { Menu, LogOut, ChevronDown, Bell, PanelLeftClose, PanelLeftOpen, Clock, X } from "lucide-react";
...
export default function StudentNavbar({ onToggleSidebar, sidebarCollapsed, onToggleCollapse }) {
  ...
  const checkDeadlines = async () => {
    try {
      ...
      for (const a of json.assignments || []) {
        ...
        if (deadline > now && deadline <= thirtyMinFromNow) {
          setDeadlineAlert(a);
          return;
        }
      }
    ...
    <>
      {deadlineAlert && (
        <motion.div
          ...
          className="bg-accent/10 border-b border-accent/20 px-4 md:px-6 py-3 flex items-center justify-between"
        >
          <div className="flex items-center gap-3">
            <Clock size={20} className="text-accent" />
            <p className="text-sm text-text-primary">
              <strong>Deadline approaching:</strong> &ldquo;{deadlineAlert.title}&rdquo; &mdash; due{" "}
              {format(new Date(deadlineAlert.deadline), "h:mm a")} (within 30 min)
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => router.push(`/student/assignments/${deadlineAlert.id}`)}
              className="text-sm bg-accent text-white px-3 py-1.5 rounded-lg font-medium hover:bg-accent/90 transition-colors"
            >
              Submit Now
            </button>
            <button
